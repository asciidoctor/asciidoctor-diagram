...
:uri-ditaa-jar: http://ditaa.sourceforge.net/
...
:uri-plantuml-jar: http://plantuml.sourceforge.net
...
Asciidoctor Diagram depends on external tools to generates images. In most cases it will locate these tools automatically for you by looking for specific executables in each directory in the `PATH` environment variable and jar files in the `CLASSPATH` environment variable.
In case you've installed a tool in a way where the executable is not in the `PATH` or a jar is not in the `CLASSPATH`, you can override its location manually using document attributes.
...
|===
   ...
   |ditaa        |{uri-ditaa-jar}[Ditaa]                                                |`ditaa-jar`
   |erd          |{uri-erd}[Erd]                                                        |`erd`
   ...
   |plantuml     |{uri-plantuml-jar}[PlantUml]                                          |`plantuml-jar`
